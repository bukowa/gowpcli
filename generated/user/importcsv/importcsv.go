...
// ImportCsv //Imports users from a CSV file.
...
func (i ImportCsv) Args() []string {
    var args = []string{"user", "import-csv"}
    args = utils.MakeArg(args, "<file>", i.File)
    args = utils.MakeArg(args, "[--send-email]", i.SendEmail)
    args = utils.MakeArg(args, "[--skip-update]", i.SkipUpdate)
    return args
}
